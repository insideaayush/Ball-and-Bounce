...
void collide121(ball a,mailbox b,int k){
    double u1x,u2x,v1x,v2x,u1y,u2y,v1y,v2y,m1,m2,n_dx1,n_dx2,n_dy1,n_dy2;
    double cosTheta,sinTheta;
    
    cosTheta=((b.x_c-a.ball_cx())/sqrt(pow(b.x_c-a.ball_cx(),2)+pow(b.y_c-a.ball_cy(),2)));
    sinTheta=((b.y_c-a.ball_cy())/sqrt(pow(b.x_c-a.ball_cx(),2)+pow(b.y_c-a.ball_cy(),2)));
    pthread_mutex_lock(&dataChangeLock);
    u1x=(a.ball_dx()*cosTheta)+(a.ball_dy()*sinTheta);
    u2x=(b.x_v*cosTheta)+(b.y_v*sinTheta);
    u1y=(a.ball_dy()*cosTheta)-(a.ball_dx()*sinTheta);
    u2y=(b.y_v*cosTheta)-(b.x_v*sinTheta);
    m1=a.ball_mass();
    m2=b.m;
    //v1x=(2*m2*u2x+(m1-m2)*u1x)/(m1+m2);
    //v2x=((m2-m1)*u2x+2*m1*u1x)/(m1+m2);
    v1x=u2x;
    v2x=u1x;
    v1y=u1y;
    v2y=u2y;
    n_dx1=v1x*cosTheta-v1y*sinTheta;
    n_dy1=v1x*sinTheta+v1y*cosTheta;
    n_dx2=v2x*cosTheta-v2y*sinTheta;
    n_dy2=v2x*sinTheta+v2y*cosTheta;
    a.ball_setdX(n_dx1);
    a.ball_setdY(n_dy1);
    B[k]=a;
    pthread_mutex_unlock(&dataChangeLock);
...
void drawCircle(ball b)//draws the circle of the ball on window
{
    ...
    GLfloat light_position[] = {2.0, 2.0, -2.0, 0.0};   //Infinite light location. */
...
void renderScene()//called every time for rendering the scene and resulting animation
{
   ...
   controlAgent=new pthread_t[n];
    for(int j=0;j<n;j++)
    {
        drawCircle(B[j]);
        if(pauseRender==0){
         pthread_create(&controlAgent[j],NULL,makeTheScene,(void*) &B[j]);
        #ifdef DEBUG
            logFile.open("lastLog.log",ios::app);
            logFile<<"ball no:"<<j<<"X pos:"<<B[j].ball_cx()<<"Y pos:"<<B[j].ball_cy()<<"Z pos:"<<B[j].ball_cz()<<endl;
            logFile.close();
        #endif
...
void ProcessSpecialKeys(int key, int x, int y) { 
    ...
    switch(key) {
        case GLUT_KEY_LEFT : // Rotate on y axis
            rotY -= 0.4f;
            break;
        case GLUT_KEY_RIGHT : // Rotate on y axis (opposite)
            rotY += 0.4f;
            break;
        case GLUT_KEY_UP : // Rotate on x axis 
            rotX -= 0.4f;
            break;
        case GLUT_KEY_DOWN : // Rotate on x axis (opposite)
            rotX += 0.4f;
            break; 
